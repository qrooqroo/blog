...
async function fetchBleepingComputer(): Promise<SecPost[]> {
  try {
...
export default async function BleepingComputerWidget({ locale = 'ko' }: { locale?: string }) {
  const posts = await fetchBleepingComputer();
        ...
        <div className="flex flex-col divide-y divide-slate-100">
          {posts.map((post, i) => (
            ...
            >
              ...
              <p className="text-[10px] text-slate-400 mt-0.5">bleepingcomputer.com</p>
            </a>
          ))}
        </div>
      )}
