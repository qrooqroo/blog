...
function makePreset(): Body[] {
  function planet(
    ...
    return {
      ...
      rotSpeed: rs, tilt: THREE.MathUtils.degToRad(tiltDeg), trail: [],
    };
  }

  const earth = planet('earth', '지구', 5, 0.65, '#4FA3D9', 13, 4.0, 23.5);
  ...
  return [
    {
      ...
      rotSpeed: 0.35, tilt: THREE.MathUtils.degToRad(7.25), trail: [],
    },
    planet('mercury', '수성', 0.5, 0.28, '#9E9E9E',  6,   3.0, 0.03),
    planet('venus',   '금성', 2,   0.55, '#E8D080',  9,  -2.5, 177.4),
    earth,
    {
      id: 'moon', name: '달', mass: 0.01, radius: 0.2, color: '#BBBBBB',
      isStar: false,
      pos: earth.pos.clone().add(new THREE.Vector3(2.0, 0, 0)),
      vel: new THREE.Vector3(0, 0, earth.vel.z - ov(2.0, earth.mass)),
      rotSpeed: 0.5, tilt: THREE.MathUtils.degToRad(6.7), trail: [],
    },
...
// ── Simulation (imperative Three.js in useFrame) ──────────────
function Simulation({ timeScale, paused, resetKey }: {
    ...
    bodies.current.forEach(b => {
      ...
      if (b.id === 'saturn') {
        ...
        const ringMat = new THREE.MeshBasicMaterial({
          color: '#C8A870', side: THREE.DoubleSide, transparent: true, opacity: 0.75,
        });
      ...
      // Uranus rings (thin, near-vertical due to extreme tilt)
      if (b.id === 'uranus') {
        const ringGeo = new THREE.RingGeometry(b.radius * 1.3, b.radius * 1.75, 64);
        const ringMat = new THREE.MeshBasicMaterial({
          color: '#7DE8E8', side: THREE.DoubleSide, transparent: true, opacity: 0.28,
        });
        const ring = new THREE.Mesh(ringGeo, ringMat);
  ...
  useFrame((_, delta) => {
    ...
    // Substeps for stability at high time scales
    ...
    for (let s = 0; s < substeps; s++) {
      const acc: THREE.Vector3[] = Array.from({ length: n }, () => new THREE.Vector3());

      for (let i = 0; i < n; i++) {
        for (let j = i + 1; j < n; j++) {
          diff.subVectors(bs[j].pos, bs[i].pos);
          const d2 = Math.max(diff.lengthSq(), 0.5);
          const f = G / d2;
          diff.normalize();
          acc[i].addScaledVector(diff, f * bs[j].mass);
          acc[j].addScaledVector(diff, -f * bs[i].mass);
        }
      }

      for (let i = 0; i < n; i++) {
        bs[i].vel.addScaledVector(acc[i], dt);
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (mesh) {
        ...
        mesh.rotation.y += bs[i].rotSpeed * Math.min(delta, 0.05) * timeScale;
      }
      ...
      <div style={{
        ...
        <button
          ...
          style={{
            ...
            background: paused ? '#16a34a' : '#475569', color: '#fff',
          }}
        >
          {paused ? '▶ 재생' : '⏸ 일시정지'}
        </button>
        <button
          onClick={() => { setResetKey(k => k + 1); setPaused(false); }}
          style={{
