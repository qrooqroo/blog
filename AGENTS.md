...
`documents` 테이블에 위키 문서를 작성하거나 수정할 때 따른다.

## 제목 작성 규칙

- `title`: 문서의 핵심 명칭만 짧게 쓴다 (예: `완전 연결 신경망`, `경사 하강법`, `Transformer`).
- `title_ko`: 한글 명칭이면 `title`과 동일하게 쓴다. 부제나 설명 문구를 붙이지 않는다.
- `title_en`: 영문 명칭만 짧게 쓴다. 콜론이나 부제 금지 (예: `Fully Connected Network`, `Gradient Descent`).

인사이트·논문 분석 글에서 쓰는 "두 절 구조 제목"은 위키 문서에 적용하지 않는다.

## 링크 작성 규칙

동일한 용어가 본문에 반복 등장할 때, 해당 용어의 위키 링크는 첫 번째 등장에만 단다. 두 번째 이후 등장에는 링크 없이 일반 텍스트로 쓴다.

## 표 작성 규칙

표(`|---|`)를 사용할 때는 반드시 표 바로 위에 해당 표의 내용을 설명하는 평문 단락을 먼저 작성한다.

- 평문은 표가 어떤 기준으로 구성됐는지, 각 열이 무엇을 의미하는지, 전체적으로 어떤 비교나 정리를 담고 있는지를 독자가 표를 읽기 전에 이해할 수 있도록 서술한다.
- 단순히 "아래 표는 X를 정리한 것이다" 같은 한 줄 안내문 수준으로 끝내지 말고, 표의 핵심 내용과 읽는 방법을 충분히 설명한다.
- 평문 없이 표만 단독으로 등장하는 구성은 허용하지 않는다.
